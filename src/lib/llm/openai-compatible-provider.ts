...
function createOpenAiCompatibleFetchOptions(targetUrl: string): FetchOptionsWithDispatcher {
  const httpProxy = readProxyEnv("HTTP_PROXY", "http_proxy");
  const httpsProxy = readProxyEnv("HTTPS_PROXY", "https_proxy");
...
function shouldBypassProxy(targetUrl: string, noProxy: string | null): boolean {
  if (!noProxy) {
    return false;
  }
